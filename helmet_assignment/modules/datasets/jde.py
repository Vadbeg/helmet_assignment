...
from helmet_assignment.modules.utils.image import (
    draw_msra_gaussian,
    draw_umich_gaussian,
    gaussian_radius,
)
from helmet_assignment.modules.utils.utils import xyxy2xywh
...
class JointDatasetHelmet(LoadImagesAndLabels):  # for training
    default_resolution = [1088, 608]
    mean = None
    std = None
    num_classes = 1
    COLUMNS_TO_USE = ['label', 'left', 'width', 'top', 'height']

    def __init__(self, opt, root, img_size=(1088, 608), augment=False, transforms=None):
        self.opt = opt
        self.img_files = OrderedDict()
        self.label_files = OrderedDict()
        self.tid_num = OrderedDict()
        ...
        self.root = root
        train_dataframe_path = os.path.join(self.root, 'train_labels_fix.csv')
        self.train_dataframe: pd.DataFrame = pd.read_csv(train_dataframe_path)

        self.train_frames = self.train_dataframe['video_frame'].unique().tolist()
        ...
        self.le.fit(self.train_dataframe['label'])

        self.nID = len(self.le.classes_) + 1

    def __getitem__(self, index):
        frame_name = self.train_frames[index]
        dataframe_part = self.train_dataframe.loc[
            self.train_dataframe['video_frame'] == frame_name
        ]
        dataframe_part = dataframe_part[self.COLUMNS_TO_USE]
        dataframe_part['label'] = self.le.transform(dataframe_part['label'])
        labels = np.c_[np.zeros(len(dataframe_part)), dataframe_part.to_numpy()]

        frame_name_parts = frame_name.split('_')
        image_name = (
            '_'.join(frame_name_parts[:-1]) + f'_frame{frame_name_parts[-1]}.jpg'
        )
        image_path = os.path.join(self.root, 'train_slices', image_name)

        imgs, labels = self.get_data(image_path, labels)
    ...
    def get_data(self, img_path, labels):
        height = self.height
        width = self.width
        img = cv2.imread(img_path)  # BGR
        if img is None:
        ...
        labels_res = labels.copy()
        labels_res[:, 2] = ratio * labels[:, 2] + padw
        labels_res[:, 3] = ratio * labels[:, 4] + padh
        labels_res[:, 4] = ratio * (labels[:, 2] + labels[:, 3]) + padw
        labels_res[:, 5] = ratio * (labels[:, 4] + labels[:, 5]) + padh

        # Augment image and labels
        if self.augment:
            img, labels_res, M = random_affine(
                img,
                labels_res,
                degrees=(-5, 5),
                translate=(0.10, 0.10),
        ...
        if nL > 0:
            # convert xyxy to xywh
            labels_res[:, 2:6] = xyxy2xywh(labels_res[:, 2:6].copy())  # / height
            labels_res[:, 2] /= width
            labels_res[:, 3] /= height
            labels_res[:, 4] /= width
            labels_res[:, 5] /= height
