...
from helmet_assignment.modules.models.common import *
...
class Model(nn.Module):
    def __init__(
        self, config='config/yolov5s.yaml', ch=3, nc=None, anchors=None
...
def get_pose_net(num_layers, heads, head_conv):
    config_file = os.path.join(
        os.path.dirname(__file__), 'networks/config/yolov5s.yaml'
    )
    pretrained = os.path.join(os.path.dirname(__file__), '../../../models/yolov5s.pt')
...
def initialize_weights(model, pretrained=''):
    ...
    if os.path.isfile(pretrained):
        ...
        print(
            ...
            % (len(state_dict), len(model.state_dict()), pretrained)
        )  # report
